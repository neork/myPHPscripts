...
class Register {
    ...
    /**
     * @var Instance of Email class
     */
    private $mailer;

    /**
     * @var Instance of Database class
     */
    private $db = null;
    ...
    public function forgotPassword($userEmail) {
        ...
        if($userEmail == "")
            $errors[] = Lang::get('email_required');
        if( ! $validator->emailValid($userEmail) )
            $errors[] = Lang::get('email_wrong_format');
        
        if( ! $validator->emailExist($userEmail) )
            $errors[] = Lang::get('email_not_exist');

        $login = new Login();

        if($login->_isBruteForce())
            $errors[] = Lang::get('brute_force');
    ...
    public function validateUser($data, $botProtection = true) {
        ...
        if( $validator->isEmpty($user['username']) )
            $errors[] = array( 
                "id"    => $id['username'],
                "msg"   => Lang::get('username_required')
            );
        ...
        if($user['password'] != $user['confirm_password'])
            $errors[] = array( 
                ...
                "msg"   => Lang::get('passwords_dont_match')
            );
        ...
        if( ! $validator->emailValid($user['email']) )
            $errors[] = array( 
                "id"    => $id['email'],
                "msg"   => Lang::get('email_wrong_format')
            );
    ...
    /**
     * Generate key used for confirmation and password reset. md5 is enough for a token key!
